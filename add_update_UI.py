...
def add_update_tab():
    ...
    with st.form(key=f"expense_form_{selected_date}"):
        ...
        for i in range(6):
            ...
            with col3:
                notes_input = st.text_input(label="Notes", value=notes, key=f"notes_{selected_date}_{i}",
        ...
        if submit_button:
            ...
            response_post = requests.post(f"{API_URL}/expenses/{selected_date}", json=filtered_expenses)
